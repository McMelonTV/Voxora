...
func runFFmpegPostProcess(ctx context.Context, ffmpegPath string, job ffmpegJob) error {
	...
	if err := cmd.Run(); err != nil {
		...
		if message != "" {
			return fmt.Errorf("ffmpeg failed (bin=%s output=%s): %w: %s", ffmpegPath, job.OutputPath, err, message)
		}
		return fmt.Errorf("ffmpeg failed (bin=%s output=%s): %w", ffmpegPath, job.OutputPath, err)
	}
	return nil
}
...
func (d *Downloader) postProcessFile(ctx context.Context, rawPath, outputPath string, metadata MediaMetadata, req Request) error {
	...
	if req.Tagging.IncludeArtwork && artworkURL != "" && canEmbedArtwork(format, outputPath) {
		var err error
		artworkPath, err = d.downloadArtworkFile(ctx, artworkURL, filepath.Dir(outputPath))
		if err != nil {
			d.log.WithError(err).Warnf("failed downloading artwork for %s", metadata.URI)
		} else {
			d.log.WithField("artwork_path", artworkPath).Debug("downloaded artwork temp file")
	...
	if err := runFFmpegPostProcess(ctx, d.ffmpegPath, ffmpegJob{
		...
		d.log.WithError(err).WithField("source", rawPath).WithField("output", tempOutputPath).Error("post-process failed")
		return err
	}
...
func (d *Downloader) downloadArtworkFile(ctx context.Context, artworkURL, tempDir string) (string, error) {
	...
	if err := os.MkdirAll(tempDir, 0o755); err != nil {
		return "", fmt.Errorf("create artwork temp dir: %w", err)
	}

	file, err := os.CreateTemp(tempDir, "libspotdl-artwork-*"+ext)
	if err != nil {
