...
func retrieveClientToken(c *http.Client, deviceId string) (string, error) {
	...
	if resp.StatusCode != 200 {
		...
		if preview != "" {
			return "", fmt.Errorf("invalid status code from clienttoken: %d (content-type=%s, body=%q)", resp.StatusCode, strings.TrimSpace(resp.Header.Get("Content-Type")), preview)
		}
		return "", fmt.Errorf("invalid status code from clienttoken: %d (content-type=%s)", resp.StatusCode, strings.TrimSpace(resp.Header.Get("Content-Type")))
	}
